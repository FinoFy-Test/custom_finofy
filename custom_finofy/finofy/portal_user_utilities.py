...
def initiate_pwd_reset():
    ...
    if user and frappe.db.exists('User', user):
        ...
        if phone:
            ...
            make(subject = subject, content=msg, recipients=user,
            send_email=True, sender= 'alerts@capitalvia.com', sender_full_name='CapitalVia')
            customer = frappe.db.get_value('Customer', {'email_id': user}, 'name')
            frappe.db.set_value('Customer', customer, {
        				'customer_otp': hotp.at(int(token))
        			})
        frappe.db.commit()
        return tmp_id
...
def confirm_device_otp_token():
    ...
    hotp = pyotp.HOTP(otp_secret)
    customer_otp = frappe.db.get_value('Customer', {'email_id': email}, 'customer_otp')

    if hotp_token:
        # raise frappe.PermissionError(int(hotp_token), "HOTP", otp)
        # if hotp.verify(otp, int(hotp_token)):
        # if otp and tmp_id and email:
        if int(otp) == customer_otp:
