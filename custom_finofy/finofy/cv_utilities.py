...
def get_subscriptions():
    ...
    subscriptions = frappe.db.sql("""
        ...
        where
            user.name = '{0}' and si.workflow_state = 'Approved'
...
def get_trading_signals():
    ...
    trading_signals = frappe.db.sql("""
        select
            ...
            sig.fumsg4_time,
            sig.message,
            sig.modified_signal as modified_message
        from
            `tabUser` user
            left join `tabCustomer` cust on cust.email_id = user.name
...
def post_rating():
    try:
        ...
        full_name, rating, comments, satisfaction, rating_service, rating_cam = frappe.form_dict.get('fullName'), frappe.form_dict.get(
            'rating'), frappe.form_dict.get('comments'), frappe.form_dict.get('satisfactionLevel'), frappe.form_dict.get('serviceRating'), frappe.form_dict.get('spRating'), frappe.form_dict.get('feedBack')
        word_ratings = {
            "5": "Excellent",
            "4": "Good",
            "3": "Average",
        ...
        first_name, sep, tail = full_name.partition(',')
        doc = frappe.get_doc({
            "doctype": "CSAT",
	...
	try:
		...
		customer = frappe.db.sql("""
			...
			where
				user.name = '{0}'
			""".format(user), as_dict=True)
		full_name, rating, comments, satisfaction, rating_service, rating_cam, feedBack = frappe.form_dict.get('fullName'), frappe.form_dict.get(
			'rating'), frappe.form_dict.get('comments'), frappe.form_dict.get('satisfactionLevel'), frappe.form_dict.get('serviceRating'), frappe.form_dict.get('spRating'), frappe.form_dict.get('feedBack')
		word_ratings = {
			"5": "Excellent",
			"4": "Good",
...
def callback_upi_wrapper(meRes, pgMerchantId):
    ...
    fcmObj.send_single_notification(
        ...
        token="cMDhrJEFTWCuLoz3bWomBN:APA91bGYZNHbi6tHcGvBOSzgd_urNLnlmVnHOBGEDvdaJu_e1di7o8tpRglXaBTLRH_r1E5sgGHqrUp55xfmGhZPdUFwt8k5jKBFAUMEOmH9oGoxAD5N0oDo9Kd43iVNNsPnZ32xEL6E"
    )
...
# commented by Omkar Jagdale - RX dependencies
# @frappe.whitelist()
...
#         where
#             user.name = '{0}' and sl.creation is not null and rx.name is not null
#         order by signal_update_on desc
#     """.format(user), as_dict=True)
...
def check_payment_status():
    user = check_permissions()
    fee_request = frappe.form_dict.get('fee_request')
    payments = frappe.get_all("UPI Payment", filters={"fee_request": fee_request}, order_by="creation desc")
    if payments:
        upi_rec = frappe.get_doc("UPI Payment", payments[0].name)
        from customer_portal_cv.customer_portal_capitalvia.upi_payment import UPIPayment
        payobj = UPIPayment()
        payobj.check_payment_transaction_status(upi_rec)
        return upi_rec.transaction_status
    else:
        return "No Payments Found"
...
def get_payment_transaction_status():
    ...
    if payments:
        ...
        if(upi_rec.transaction_status == 'SUCCESS' and upi_rec.fcm_token):
            from customer_portal_cv.customer_portal_capitalvia.fcm_utils import FcmUtils
            fcmObj = FcmUtils()
            fcmObj.send_single_notification(
                message={
        ...
        if(upi_rec.transaction_status == 'FAILED' and upi_rec.fcm_token):
            from customer_portal_cv.customer_portal_capitalvia.fcm_utils import FcmUtils
            fcmObj = FcmUtils()
            fcmObj.send_single_notification(
                message={
...
@frappe.whitelist()
def check_payment_status_response():
    user = check_permissions()
    fee_request = frappe.form_dict.get('fee_request')
    payments = frappe.get_all("UPI Payment", filters={"fee_request": fee_request}, order_by="creation desc")
    if payments:
        ...
        from customer_portal_cv.customer_portal_capitalvia.upi_payment import UPIPayment
        payobj = UPIPayment()
        return upi_rec, payobj.check_transaction_response(upi_rec)
        # return upi_rec.transaction_status
    else:
        return "No Payments Found"
